...
export default async function NewsMediaPage() {
  ...
  if (!templateData || !sectionData?.newsMedia?.variants?.LexoraNewsMedia1) {
    return <div>Data not found</div>;
  }

  const globalUI = sectionData?.globalUI?.variants?.LexoraGlobalUI1;
  const breadcrumbData = sectionData?.newsMediaBreadcrumb?.variants?.LexoraNewsMediaBreadcrumb1;
  ...
  return (
    <main className="bg-white">
      ...
      <NewsMediaGrid data={sectionData?.newsMedia?.variants?.LexoraNewsMedia1} />
